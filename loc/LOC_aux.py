        ...
        if (1):
            ...
            for ipartner in range(1, self.PARTNERS):
                ...
                if (self.CUL[0].shape[0]!=self.CUL[ipartner].shape[0]):
                    ...
                    for i in range(rows):
                        if ((self.CUL[0][i][0]!=self.CUL[ipartner][i][0])|(self.CUL[0][i][1]!=self.CUL[ipartner][i][1])):
                            redo = True
...
def ReadIni(filename):
    ...
    for line in lines:
        ...
        if (s[0].lower().find("fits")==0): # 0 or 2 arguments
            INI['FITS'] = 1
            if (len(s)==2):                # one can also switch FITS off
                INI['FITS'] = int(s[1])
            if ((len(s)>2)&(s[1]!="#")):
...
def InitCL(GPU=0, platforms=[], idevice=0, sub=0, verbose=True):
    ...
    """
    cache_key = (GPU, tuple(platforms), idevice, sub)
    if cache_key in _INITCL_CACHE:
        if (verbose and _verbose()):
            platform, device, context, queue, mf = _INITCL_CACHE[cache_key]
            print("  Reusing OpenCL context")
            print("  Platform: ", platform)
            print("  Device:   ", device)
        return _INITCL_CACHE[cache_key]
    ...
    for iplatform in possible_platforms:
        ...
        try:
            ...
            errors.append("platform %d: %s" % (iplatform, err))
            platform, device, context, queue = None, [], None, None
    # print("***InitCL completed***")
    if (context is None) or (queue is None) or (len(device)<1):
        available = []
        try:
            for i, p in enumerate(cl.get_platforms()):
                names = []
                for dtype, label in ((cl.device_type.CPU, "CPU"), (cl.device_type.GPU, "GPU")):
                    try:
                        names += ["%s:%s" % (label, d.name) for d in p.get_devices(dtype)]
                    except Exception:
                        pass
                available.append("platform %d %s devices=[%s]" % (i, p.name, ", ".join(names)))
        except Exception as err:
            available.append("could not list OpenCL platforms: %s" % err)
        raise RuntimeError(
            "InitCL failed: requested %s device index %d from platforms %s.\nAvailable OpenCL devices:\n%s\nErrors:\n%s" %
            (["CPU", "GPU"][GPU>0], idevice, list(possible_platforms),
             "\n".join(available), "\n".join(errors)))
    if (verbose):
        print("  Platform: ", platform)
        print("  Device:   ", device)
    _INITCL_CACHE[cache_key] = (platform, device, context, queue, cl.mem_flags)
    return _INITCL_CACHE[cache_key]


def BuildProgram(context, source, options=None):
    cache_key = (id(context), source, options)
    if cache_key not in _PROGRAM_CACHE:
        program = cl.Program(context, source)
        if options is None:
            _PROGRAM_CACHE[cache_key] = program.build()
        else:
            _PROGRAM_CACHE[cache_key] = program.build(options)
    elif _verbose():
        print("  Reusing compiled OpenCL program")
    return _PROGRAM_CACHE[cache_key]


def BuildKernel(program, name):
    cache_key = (id(program), name)
    if cache_key not in _KERNEL_CACHE:
        _KERNEL_CACHE[cache_key] = cl.Kernel(program, name)
...
class BandO:
    ...
    def Init(self, bandwidth, width):
        ...
        self.VMIN       = 0.0
        self.VMAX       = 0.0
        self.VELOCITY   = asarray([], float32)
        self.WEIGHT     = asarray([], float32)
...
def ConvolveSpectra1D(filename, fwhm_as, GPU=0, platforms=[0,1,2,3,4], angle_as=-1.0, samples=201):
    ...
    program      =  BuildProgram(context, source)
    kernel_con   =  BuildKernel(program, 'Convolve')
...
def ConvolveCube(S, fwhm, GPU, platforms):
    ...
    program      =  BuildProgram(context, source, opt)
    kernel_con   =  BuildKernel(program, 'Convolve3D')
